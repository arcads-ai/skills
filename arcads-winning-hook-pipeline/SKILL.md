---
name: arcads-winning-hook-pipeline
description: >
  ...
  sequence: arcads-spy-competitor-ads → arcads-hook-identifier → arcads-hook-cloner. Use this skill whenever
...
The three acts:
1. **Find** — `arcads-spy-competitor-ads` discovers and downloads the top 5 competitor videos
2. **Identify** — `arcads-hook-identifier` extracts the hook from the chosen video
3. **Clone** — `arcads-hook-cloner` recreates the hook for the user's brand

---

## Act 1 — Find winning ads

Load the `arcads-spy-competitor-ads` skill:

```
mcp_Skill("arcads-spy-competitor-ads")
```
...
**Handoff**: After the 5 ads are presented, do not offer the standard arcads-spy-competitor-ads next-step menu. Instead, ask with `mcp_Question`:
...
Load the `arcads-hook-identifier` skill:

```
mcp_Skill("arcads-hook-identifier")
```
...
Load the `arcads-hook-cloner` skill:

```
mcp_Skill("arcads-hook-cloner")
```

The hook analysis from Act 2 is already in context — `arcads-hook-cloner` will pick it up automatically (it handles the "arcads-hook-identifier output already in conversation" entry point). Follow its instructions: collect brand info, adapt the script, generate with Seedance 2.0.

**Note on brand info**: if the user already described their brand during Act 1 (arcads-spy-competitor-ads asks for brand + category), carry that context forward. `arcads-hook-cloner` should skip questions already answered.
