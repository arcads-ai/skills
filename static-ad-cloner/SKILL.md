---
name: static-ad-cloner
description: >
  Clone a static (image) ad for the user's own brand. Optionally takes a reference static
  ad as an image — if none is provided, automatically runs the spy-competitor-ads skill
...
Do not stop and ask "which static ad?". Instead, run the **`spy-competitor-ads` skill in static mode** to source candidate references from the Meta Ad Library, then pick one to clone:

1. Trigger the `spy-competitor-ads` skill explicitly for **static / image ads** (it has a built-in static mode that uses `media_type=image`). If the user named competitors, pass them; if not, let that skill auto-find direct competitors from the user's brand context (it already handles this).
...
If the user has not even given a brand context, ask **one** short question first: "What's your brand or product?" — then trigger spy-competitor-ads in static mode with that.

**C. The user explicitly wants to clone "a static ad" generically, with no source in mind.**
Treat this as case B — auto-source via spy-competitor-ads (static mode). Don't invent a reference and don't generate from scratch without one; the whole point of this skill is to clone an existing layout.
...
- **No reference static ad yet**: do **not** stop. Trigger the `spy-competitor-ads` skill in static mode to source one automatically (see Step 1B). Only ask the user for help if there's no brand/product context to drive the search.
- **spy-competitor-ads returns no static ads** for the chosen competitors: try one more set of competitors if the user gave brand context to work with, otherwise stop and ask the user to share a reference static ad directly.
...
| Tool | Where |
|---|---|
| `spy-competitor-ads` skill (static mode) | Step 1B — auto-source a reference static ad when the user didn't provide one |
